...
//    
//      listDir(SPIFFS, "/", 0);    
//      if (count_main== 0) appendFile(SPIFFS, "/log.txt", "\nNewLog\n");
...
{     delay(15000);
      ...
      Serial.println("");
      // -- COMPRESSÃO--
      for(int qtd_msn=1;qtd_msn<20;qtd_msn++){// Laço que determina a quantidade de mensagem por compressão, indo de 1 a grupos de 20.
      Serial.printf("qtd msn %d \n",qtd_msn);
       for(int grop=0;grop<5;grop++){ // LAÇO QUE DETERMINA QUANTAS VEZES SERÁ REALIZADA AQUELE GRUPO DE COMPRESSÕES
        for(int i=0;i<1000;i++){ // NÚMERO DE COMPRESSÕES CONSECUTIVAS 
        //Serial.println(i);  
        free_heap=99999999;
        int aux_heap=ESP.getFreeHeap();  
        //Serial.println(aux_heap);
