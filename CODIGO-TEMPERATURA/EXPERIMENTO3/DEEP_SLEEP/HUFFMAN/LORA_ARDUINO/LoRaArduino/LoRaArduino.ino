...
RTC_DATA_ATTR int count_msn = 0; 
RTC_DATA_ATTR int pacote_enviado=0;
RTC_DATA_ATTR char mensagem_env[25][55];
...
uint8_t datasend[36];
RTC_DATA_ATTR int count_main = 0; 
...
void setup()
{
     ...
     // LoRa.setTxPower(20, PA_OUTPUT_RFO_PIN);
      ...
      //LoRa.receive();  
      esp_sleep_enable_timer_wakeup(tempo * uS_TO_S_FACTOR);
...
void sendWifiData() {
  ...
  ThingSpeak.setField(5, count_main);
...
void max6675Write()
{
    ...
    dtostrf(count_main, 0,0, count_1);
...
void compress_aux(unsigned char **mensagem2,int *tamanho,int qtd_msn){ 
        ...
        unsigned char t[TAB_OFFSET-1]={0};
       ...
       unsigned  int tamanho_comprimido=0;
       int var_tab = tabela(var,t); // CRIAÇÃO DA TABELA
      
       if(var_tab!=0){
          Serial.println("Estou de buffer");
          ESP.restart();
        }
        
       lista *ordena= (lista*) malloc(sizeof(lista));
       if(ordena==NULL){
          free(mensagem);
          free(var);
          Serial.println("Estou de Memória");
           ESP.restart();
        }
       constroi_lista(t,ordena); // CONSTROI LISTA ORDENADA A PARTIR DA TABELA
    
       constroi_arvore(ordena); //////////// CONSTROI A ARVORE DE HUFFMAN
      
       // Constroi igual ao do livro
    
        int soma=strlen(var); //////// TAMANHO DO DADO DESCOMPACTADO
       // TAMANHO DA VARIAVEL EM BYTES
        unsigned char bits=7; // QUANTOS BITS FORAM COMPLETADOS
        unsigned char aux_cod=0;
    
        compress_dado(var,&tamanho_comprimido,&bits,&aux_cod,mensagem,ordena->head->dado);
    
        if(bits!=7){
            mensagem[TAB_OFFSET-1]=7-bits; // SALVA A QUANTIDADE DE BITS A CONSIDERAR NO ULTIMO
            mensagem[tamanho_comprimido+TAB_OFFSET]=aux_cod;
    
            tamanho_comprimido++;}
        else{
           // printf("NENHUM BYTE INCOMPLETO \n");
            mensagem[TAB_OFFSET-1]=0;  // SENAO SOBRE NENHUM BYTE INCOMPLETO COLOCA AQUI
        }
        // CARREGA TABELA
       ...
       //----- DESCOMPRESSÃO
//       char *mensagem_descompress;
//       tamanho_comprimido=tamanho_comprimido+TAB_OFFSET;
//       descompacta(&mensagem_descompress,mensagem,tamanho_comprimido,qtd_msn);
//    
//       int comparacao=strcmp(mensagem_descompress,var);
//      //int comparacao=0;
//       float tx=(float)(100*tamanho_comprimido/strlen(var))/100;
//       
//       Serial.printf("TAGBUSCA /%d/%d/%d/%f/%ld/%d/%d/%ld /%ld \n",qtd_msn,strlen(var),tamanho_comprimido,100*tx,stop,comparacao,0,uxTaskGetStackHighWaterMark( NULL ));
//       free(mensagem_descompress);
    //   free(mensagem);
       ...
       FreeHuffmanTree(ordena->head->dado); // APAGA ARVORE OS NOS DA ARVORE
       free(ordena->head); // APAGA ESSE ELEMENTO
     
       free(ordena);
        *mensagem2=mensagem;
       *tamanho=tamanho_comprimido+TAB_OFFSET;
...
void loop()
{     
      int qtd_pacotes[2]={16,5}; //SF 7 - QUANTIDADE DE MENSAGEM POR PACOTE
      int pacote=2; // QUANTIDADE DE PACOTE
//    int qtd_pacotes[7]={3,3,3,3,3,3,3};//SF 12 - QUANTIDADE DE MENSAGEM POR PACOTE
//    int pacote=7; 

if(count_msn==0 && pacote_enviado==0 ){
        delay(15000);
        Serial.println("RESANDO CONFIG");
        esp_sleep_enable_timer_wakeup(tempo * uS_TO_S_FACTOR);
        
        }
      unsigned char *mensagem; // MENSAGEM COMPRIMIDA
      int tam=0; // TAMANHO DA MENSAGEM COMPRIMIDA
      for (int i=pacote_enviado;i<pacote;i++){
        Serial.printf("COUNT %d  I %d  MAX TO SEND %d\n",count_msn,i,qtd_pacotes[i]);

        while(count_msn<qtd_pacotes[i]){
          count_msn++;
          get_men_fix(count_msn-1,*(mensagem_env+count_msn-1)); // PRECISA SER ASSIM PARA PONTEIRO DE PONTEIRO
          Serial.printf("COUNT %d %s\n",count_msn,*(mensagem_env+count_msn-1));
      ...
      free(mensagem);
       ...
       Serial.println("REINICIA PACOTE");
      pacote_enviado=0;
