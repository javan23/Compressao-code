...
void compress_aux(unsigned char **mensagem2,int *tamanho,int qtd_msn){ 
       ...
       if(dic_list==NULL){
        ...
        if(get_msn(&var,&mensagem,qtd_msn))
        {
          ...
          Serial.println("QUEBRA DE CODIGO");
          free(dic_list);
          ESP.restart();
         }
       ...
       int compress_vlr=COMPRIME_LZ78(dic_list,var,strlen(var),mensagem,&tamanho_comprimido);
        if(compress_vlr!=0){
            free(var);
            free(mensagem);
...
//       DESCOMPRIME_LZ78(mensagem,mensagem_descompress,tamanho_comprimido);
//      
...
//       free(mensagem_descompress);
     ...
     // Serial.println("PROBLEMA AQUI 2");
      delete_list(dic_list);
       *mensagem2=mensagem;
...
void loop()
{     
      int qtd_pacotes[2]={11,10};//SF7 - QUANTIDADE DE MENSAGEM POR PACOTE
      ...
      for (int i=pacote_enviado;i<pacote;i++){
        ...
        count_msn++;
        get_men_fix(count_msn-1,*(mensagem_env+count_msn-1)); 
