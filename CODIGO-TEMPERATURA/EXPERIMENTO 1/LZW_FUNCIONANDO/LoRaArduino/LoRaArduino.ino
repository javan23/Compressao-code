...
{     delay(5000);
      ...
      for(int qtd_msn=1;qtd_msn<40;qtd_msn++){ // Laço que determina a quantidade de mensagem por compressão, indo de 1 a grupos de 39.
        ...
        lista *dic_list= (lista *) malloc(sizeof(lista)); // Declaração da lista utilizada como dicionário.
        if(dic_list==NULL)
          continue;
        constroi_lista(dic_list);
        
        char dic[DIC_SIZE]={0};  // Dicionário inicial do LZW
        ...
        unsigned long  start =micros(); // Inicia a aferição do tempo
        ...
        for(int i=0;i<DIC_SIZE-1;i++)
            insere_elemento_lista_ordenado(dic_list,dic+i,1); // Carrega dicionário inicial
       ...
       int aux_size=dic_list->size_dic; // Ver tamanho do dicionário 
      
       //----- DESCOMPRESSÃO
       char *mensagem_descompress= (char*) calloc(35*qtd_msn,sizeof(char));

        if(mensagem_descompress==NULL)
       {  Serial.println("Falha na alocação");
          free(var);
          free(mensagem);
          delete_list(dic_list);
          continue;
       ...
       compress_vlr=DESCOMPRIME_LZW(mensagem,mensagem_descompress,tamanho_comprimido);
       ...
       //Serial.printf("\n  ORIGINAL %s \n DESCOMPRIMIDA %s",var,mensagem_descompress);
      ...
      if(count < NO_SLEEP_RUN) {   
          ...
          Display.setCursor(0,36); Display.print("T2: " +(String)aux_size);
