...
RTC_DATA_ATTR char mensagem_env[20][55];
...
//      int pacote=3;
   ...
   if(count_msn==0 && pacote_enviado==0 ){
      ...
      for (int i=pacote_enviado;i<pacote;i++){
        ...
        count_msn++;
        get_men_fix(count_msn,*(mensagem_env+count_msn-1)); // PRECISA SER ASSIM PARA PONTEIRO DE PONTEIRO
        //Serial.printf("COUNT %d %s\n",count,*(mensagem_env+count_msn-1));
        esp_deep_sleep_start();
        
         }
      count_msn=0;
      pacote_enviado++;
      compress_aux((char**) &mensagem,&tam,qtd_pacotes[i]);
       LoRa.beginPacket();
